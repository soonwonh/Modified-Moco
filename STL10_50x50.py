...
if args.results_dir == '':
    args.results_dir = './50x50_album-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S-moco")

#print(args)

#args.resume = '/home/sunwon/moco-test/moco-test1/multicrop_twoloss_v5_89.87%/model_last.pth'
...
train_transform = A.Compose([
    A.RandomResizedCrop(always_apply=False, p=1, height=50, width=50, scale=(0.08,1.0),ratio=(0.75,1.33333333), interpolation=2),
    ...
    A.Normalize(mean=(0.4914, 0.4822, 0.4465),std=(0.2471, 0.2435, 0.2616),max_pixel_value=255.0,always_apply=True,p=1.0),
    ToTensorV2()
])
...
train_data2 = datasets.ImageFolder(
        traindir,
        loader.TwoCropsTransform(train_transform))
"""
...
# data prepare
train_data = STL10Pair(root='data', split='train', transform=train_transform, download=True)
train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)

memory_data = STL10(root='data', split='train', transform=test_transform, download=True)
...
class ModelMoCo(nn.Module):
    ...
    def forward(self, im1, im2):
        ...
        if self.symmetric:  # asymmetric loss
            ...
            #print("torch.cat([k1, k2], dim=0).shape", k.shape)
